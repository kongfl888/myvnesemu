...
void	CLauncherDlg::Destroy()
{
	if( m_hWnd ) {
		...
		if( !m_FileListNum ) {
			...
			::strcpy_s( Config.launcher.szLastSelect, sizeof(Config.launcher.szLastSelect), Path.c_str() );
		}
...
//DEBUGOUT( "L1:%08X L2:%08X PA:%d\n", lParam1, lParam2, lParamSort );
	...
	switch( lParamSort ) {
		case	COLUMN_FILENAME:
#if	1
			::strcpy_s( szTemp, sizeof(szTemp), fl1.fname.c_str() );
			s1 = (CHAR*)::_mbsupr( (UCHAR*)szTemp );
			::strcpy_s( szTemp, sizeof(szTemp), fl2.fname.c_str() );
			...
			return	ret;
		case	COLUMN_PATH:
			::strcpy_s( szTemp, sizeof(szTemp), fl1.path.c_str() );
			s1 = (CHAR*)::_mbsupr( (UCHAR*)szTemp );
			::strcpy_s( szTemp, sizeof(szTemp), fl2.path.c_str() );
			s2 = (CHAR*)::_mbsupr( (UCHAR*)szTemp );
			if( !Config.launcher.bSortDir ) {
				ret = s1.compare( s2.c_str() );
			} else {
...
_Compare_Str:
#if	1
			::strcpy_s( szTemp, sizeof(szTemp), s1.c_str() );
			s1 = (CHAR*)::_mbsupr( (UCHAR*)szTemp );
			::strcpy_s( szTemp, sizeof(szTemp), s2.c_str() );
			s2 = (CHAR*)::_mbsupr( (UCHAR*)szTemp );
			if( !Config.launcher.bSortDir ) {
				ret = s1.compare( s2.c_str() );
			} else {
...
void	CLauncherDlg::SetListView( INT index, FILELIST& fl )
{
	...
	if( Config.launcher.bHeaderView[2] ) {
		...
		if( Mapper == 20 ) {
			::strcpy_s( szStr, sizeof(szStr), "FDS" );
			if( fl.mapper&0xF000 )
				image = 4;
			else
				image = 3;
		} else if( Mapper == 0x100 ) {
			::strcpy_s( szStr, sizeof(szStr), "NSF" );
			image = 5;
		} else if( Mapper > 0x100 ) {
			::strcpy_s( szStr, sizeof(szStr), "Unknown" );
			image = 0;
		} else {
...
//DEBUGOUT( "Item double click!! SEL=%08X\n", m_SelectPos );
		FILELIST& fl = m_FileList[m_SelectPos];
		string	path = CPathlib::MakePath( fl.path.c_str(), fl.fname.c_str() );
		::strcpy_s( m_LaunchPath, sizeof(m_LaunchPath), path.c_str() );
