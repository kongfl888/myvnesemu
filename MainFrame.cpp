...
//	m_uKeyTimerID = ::SetTimer( m_hWnd, 0x0100, 30, NULL );	// 30ms
	...
	if( ::strlen( CApp::GetCmdLine() ) > 0 ) {
		...
		if( pCmd[0] == '"' ) {	// Shell execute!!
			...
			::strcpy_s( m_szCommandLine, sizeof(m_szCommandLine), pCmd );
		}
...
//				throw	CApp::GetErrorString( IDS_ERROR_UNSUPPORTFORMAT );
		...
		::strcpy_s( szErrorString, sizeof(szErrorString), str );
		PostMessage( m_hWnd, WM_VNS_ERRORMSG, 0, (LPARAM)szErrorString );
#ifndef	_DEBUG
	} catch(...) {
		// 不明なエラーが発生しました
		::strcpy_s( szErrorString, sizeof(szErrorString), CApp::GetErrorString( IDS_ERROR_UNKNOWN ) );
		PostMessage( m_hWnd, WM_VNS_ERRORMSG, 0, (LPARAM)szErrorString );
#endif
	}
}
...
//	DEBUGOUT( "CMainFrame::OnEmuCommand %d\n", uID );
	...
	switch( uID ) {
		...
		case	ID_FILTER_XBRZ2X:
			...
			goto	_gohell;
		case	ID_FILTER_SCANLINES:
			Emu.EventParam( CEmuThread::EV_MESSAGE_OUT, (LONG)"Filter: Scanlines" );
			goto	_gohell;
		case	ID_FILTER_TVLINES:
			Emu.EventParam( CEmuThread::EV_MESSAGE_OUT, (LONG)"Filter: TV" );
			goto	_gohell;
		case	ID_FILTER_PIXELATE:
			Emu.EventParam( CEmuThread::EV_MESSAGE_OUT, (LONG)"Filter: Pixelate" );
			goto	_gohell;
