...
CDirectDraw::BLTFUNC	CDirectDraw::nxScanlinesBltTable[] = {
	&CDirectDraw::Render8bpp_Double,
	&CDirectDraw::nx_scanlines_16bpp,
	&CDirectDraw::Render24bpp_Double,
	&CDirectDraw::nx_scanlines_32bpp,
};
CDirectDraw::BLTFUNC	CDirectDraw::nxTVlinesBltTable[] = {
	&CDirectDraw::Render8bpp_Double,
	&CDirectDraw::nx_tvlines_16bpp,
	&CDirectDraw::Render24bpp_Double,
...
DDSURFACEDESC2	ddsd;
	...
	//m_lpRender is not empty, add it to dx layer
	LPBYTE	lpRdr = m_lpRender+8;
	
	if(g_bSan2)
	ViewChars(lpRdr);//add san guo zhi 2 HA
	...
	if( !m_nBltFilter || !IsMMX() ) {
		...
		switch( m_nBltFilter ) {
			...
			case	BLTFILTER_XBRZ2X:
				...
				break;
			case	BLTFILTER_SCANLINES:
				bltfunc = nxScanlinesBltTable;
				break;
...
void	CDirectDraw::ViewChars( unsigned char * lpRdr )	//draw float text.
{

	//CPU_BACKUP[] can get memory of backup (0-ff)
	...
	switch(CPU_BACKUP[0x60])
		{
			...
			case 0x5	:	//end fighting
				if(CPU_BACKUP[0xBA] == 0x08)
				{
					...
					{
						Line_Start = 20;			//scanner
						gridy = 32 - Line_Start;	//offset of scanner
					}
					if(CPU_BACKUP[0xBB] != 0x09)
					{
						MAX_INDEX_R = MIN_INDEX;
