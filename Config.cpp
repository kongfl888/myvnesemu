...
// Launcher
	...
	for( i = 0; i < 16; i++ ) {
		...
		if( CRegistry::GetProfileString( section.c_str(), keys, szTemp, sizeof(szTemp) ) )
			::strcpy_s( launcher.szFolder[i], sizeof(launcher.szFolder[i]), szTemp );
	}

	if( CRegistry::GetProfileString( section.c_str(), "LastSelect", szTemp, sizeof(szTemp) ) )
		::strcpy_s( launcher.szLastSelect, sizeof(launcher.szLastSelect), szTemp );
...
// ExtraSound
	...
	for( i = ESF_MOEPRO_STRIKE; i <= ESF_MOEPRO_WA; i++ ) {
		...
		if( CRegistry::GetProfileString( section.c_str(), keys, szTemp, sizeof(szTemp) ) )
			::strcpy_s( extsound.szExtSoundFile[i], sizeof(extsound.szExtSoundFile[i]), szTemp );
	}
...
// NetPlay
	...
	if( CRegistry::GetProfileString( section.c_str(), "NickName", szTemp, sizeof(szTemp) ) )
		::strcpy_s( netplay.szNick, sizeof(netplay.szNick), szTemp );
	...
	for( i = 0; i < netplay.nRecentPort; i++ ) {
		...
		if( CRegistry::GetProfileString( section.c_str(), keys, szTemp, sizeof(szTemp) ) )
			::strcpy_s( netplay.szRecentPort[i], sizeof(netplay.szRecentPort[i]), szTemp );
	}
	...
	for( i = 0; i < netplay.nRecentHost; i++ ) {
		...
		if( CRegistry::GetProfileString( section.c_str(), keys, szTemp, sizeof(szTemp) ) )
			::strcpy_s( netplay.szRecentHost[i], sizeof(netplay.szRecentHost[i]), szTemp );
	}
}
