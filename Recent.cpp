...
LPCSTR	CRecent::GetName( INT nID )
{
	...
	::strcpy_s( m_TempPath, sizeof(m_TempPath), m_RecentName[nID] );
...
void	CRecent::MakeManuPath( LPSTR lpszPath )
{
	...
	if( ::strlen( FileName.c_str() ) >= 30 ) {
		::strcpy_s(lpszPath, strlen(lpszPath) + 1, FileName.c_str());
		return;
	}
	...
	if( ::strlen(m_RecentPath[0]) <= 0 ) {
		...
		for( i = 0; i < RECENT_MAX; i++ ) {
			if( ::strlen(m_RecentPath[i]) > 0 ) {
				...
				::strcpy_s( szRecent, sizeof(szRecent), m_RecentPath[i] );
	...
	if( ::strlen(m_RecentName[0]) <= 0 ) {
		...
		for( i = 0; i < RECENT_MAX; i++ ) {
			if( ::strlen(m_RecentName[i]) > 0 ) {
				// Shorten paths for menus
				::strcpy_s( szRecent, sizeof(szRecent), m_RecentName[i] );
...
void	CRecent::Add( LPCSTR lpszPath )
{
	...
	if( ::strlen(m_RecentName[0]) > 0 ) {
		...
		if( j == RECENT_MAX )
			j--;
		for( ; j > 0; j-- ) {
			::strcpy_s(&m_RecentName[j][0], sizeof(m_RecentName[j]), &m_RecentName[j - 1][0]);
		}
	}
	::strcpy_s( m_RecentName[0], sizeof(m_RecentName[0]), lpszPath );
	...
	if( ::strlen(m_RecentPath[0]) > 0 ) {
		...
		if( j == RECENT_MAX )
			j--;
		for( ; j > 0; j-- ) {
			::strcpy_s( m_RecentPath[j], sizeof(m_RecentPath[j]), m_RecentPath[j-1] );
		}
	}
	::strcpy_s( m_RecentPath[0], sizeof(m_RecentPath[0]), temp.c_str() );
}
...
void	CRecent::Load()
{
	...
	for( i = 0; i < RECENT_MAX; i++ ) {
		...
		if( CRegistry::GetProfileString( "Recent Path List", szEntry, szTemp, sizeof(szTemp) ) )
			::strcpy_s( m_RecentPath[i], sizeof(m_RecentPath[i]), szTemp );
	}
	for( i = 0; i < RECENT_MAX; i++ ) {
		::wsprintf( szEntry, "File%d", i+1 );
		if( CRegistry::GetProfileString( "Recent File List", szEntry, szTemp, sizeof(szTemp) ) )
			::strcpy_s( m_RecentName[i], sizeof(m_RecentName[i]), szTemp );
	}
}
